...
class RateLimiter(BaseRedisClient):
    ...
    Example:
        ```python
        # Check rate limit
        key = rate_limiter.build_rate_limit_key("post", "create", f"user:{user_id}")
    ...
    def build_rate_limit_key(self, resource: str, action: str, identifier: str) -> str:
        """
        Build a consistent rate limit key.

        Args:
            resource: The resource being rate limited (e.g., "post", "vote", "auth")
            action: The action being performed (e.g., "create", "update", "login")
            identifier: The identifier (e.g., "user:123", "ip:192.168.1.1")

        Returns:
            str: Formatted rate limit key

        Example:
            >>> build_rate_limit_key("post", "create", "user:123")
            "ratelimit:post:create:user:123"
        """
        return f"ratelimit:{resource}:{action}:{identifier}"
    ...
    async def check_rate_limit(
        ...
        try:
            ...
            reset_time = int(time.time()) + window  # Unix timestamp in seconds
